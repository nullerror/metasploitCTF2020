...
### The script found an ASCII file
...
This was a bit tougher to solve. Probably more of a *middle-finger*, right? RIGHT??? Ha... ehh....

### The /root/ folder had file called 'hai'. 
...
*kali:$ git clone ./hai unbundled-directory*
...
No it wasn't. This is a ruse. A horrible, evil ruse. It's just half of a card! I had a chuckle at this, honestly.

<img src="half_card.png" width="200">

At this point I moved on to make more progress on some of the other cards that I may or may not have already mentioned. Once I came back and realized that the other pieces are probably just hidden in the bundle, I was able to start digging further into the trenches of git objects, blobs, hashes, and commits.

### Take a look at .git/config

There is another branch listed called 'nothing-to-see-here' mentioned in the config file. I took note of that for later. I'm no betting man, but I bet to myself, 'self, I bet that there IS something to see there'. First I wanted to make sure my fleeting attention span didn't get distracted, so I looked through the 'master' branch to see if there were any other clues. I tried *'git log'* but there was only one commit for master. Next I did a  *'git rev-list --all'* and it showed more hashes so there was obviously more stuff here.
...
was fairly simple. The 'whats-this' file from 'master' was the first piece since it had the file signature on it, and the last piece was the base64 decoded part retrieved from the 'nothing-to-see-here' branch. It had the typical ending of the flags, which was metadata about the image. It also had IEND, which all PNGs have. The 'final' piece, (assuming it was the 'only middle') was placed in the middle. The files were stiched together quicky with 'cat' and the hash submitted successfully.
...
Tail was showing the very end of a PNG file. We then tried 'head -c 100' and narrowed in on 'head -c 1050' which was the maximum amount of bytes that could be read from the stream at once. This showed the typical PNG signature. So we were getting close.
